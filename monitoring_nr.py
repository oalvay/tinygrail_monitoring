...
whitelist = []
temp = ''
print("\n现在，你需要依次输入一列无需监测的用户白名单。白名单用户越多，脚本运行速度越快。")
print("\n>>完成后，输入 done 来结束<<\n")
while temp != 'done':
...
sleeping = try_until("\n你希望脚本多久监测一次？输入秒数（推荐至少10秒，秒数越低越不稳定）：")
...
def check_cash(holders):
    urls = [f'{api}chara/user/assets/{uid}' for uid in holders]
    results = [round(get_text(url)['Value']['Balance'],2) for url in urls]
    return results
